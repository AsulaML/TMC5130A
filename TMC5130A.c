...
void Stepper_Config_uStep_Pos_Direction(uint8_t WichStepper, int32_t microstep_number, uint8_t rot_dir, int32_t offset)
{
    ...
    while(time--) 
    {
        delay_ms(1);
				...
				{
					case STEPPER_X :
						if (!Is_Motor_Stopped(STEPPER_X)) return;
					break;

					case STEPPER_Y :
						if (!Is_Motor_Stopped(STEPPER_Y)) return;
					break;

					case STEPPER_Z :
						if (!Is_Motor_Stopped(STEPPER_Z)) return;
					break;
				}
...
bool Is_Motor_Stopped(uint8_t WichStepper) 
{
    ...
    if (Is_Motor_Velocity_Zero(WichStepper))
    {
        return true;
    }
...
bool Is_Motor_Velocity_Zero(uint8_t WichStepper)
{
    ...
    uint32_t flags = Read_RAMP_STAT(WichStepper);
